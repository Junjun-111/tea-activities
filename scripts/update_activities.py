...
def fetch_city_activities(city, news, today, cutoff_days=7):
    ...
    def build_search_prompt(brands_text):
        return (
            ...
            + "【信息来源优先级】优先检索品牌官方渠道：官方微博、微信公众号、小红书官方账号、"
            "品牌官网/小程序公告，其次是茶饮行业媒体（饮品报等）。"
            "官方账号最近 1-3 天刚发布的新品/联名/买赠，即使媒体还没报道也必须收录；"
            "官方文案里的非常规叫法（如「XX Pro」「明天报到」「超多肉」「多肉加倍」）"
            "也要当作新品线索去核实。\n"
            "【执行步骤】清单里每个品牌都要单独搜，而且**每个品牌至少搜 2 次**"
            "（一次搜「品牌名 新品」，一次搜「品牌名 联名/买一送一/上新」），"
            f"逐个核实最近 {cutoff_days} 天内是否有以下活动：\n"
            "1. 新品上市（含老款回归、升级版、季节限定）；\n"
            "2. IP联名/限定联名产品；\n"
