...
# 免费模式的规则关键词 —— 用户口径：只要这三类
#   ① 新品（含上新/首发/回归/升级/限定）
#   ② 联名 / 合作款
#   ③ 送实物周边/物料（杯套、贴纸、冰箱贴、徽章…）
# 优惠类（买一送一、第二杯、折扣）、纯门店活动（开业、快闪）、纯打卡 都不再算
NEW_KEYWORDS = ("新品", "上新", "上市", "首发", "回归", "升级", "限定", "Pro", "报到",
                "新口味", "新系列", "新推")
COLLAB_KEYWORDS = ("联名", "联乘", "合作款", "×")
GOODS_KEYWORDS = ("周边", "物料", "杯套", "贴纸", "冰箱贴", "徽章", "吧唧", "明信片",
                  "帆布袋", "挂件", "杯垫", "挂饰", "公仔", "钥匙扣", "礼盒")
# 用户明确要保留：买一送一（含买1送1、第二杯半价这类门店直享优惠）
DEAL_KEYWORDS = ("买一送一", "买1送1", "第二杯", "半价")
ACT_KEYWORDS = NEW_KEYWORDS + COLLAB_KEYWORDS + GOODS_KEYWORDS + DEAL_KEYWORDS
...
NOISE_KEYWORDS = ("有哪些", "盘点", "汇总", "合集", "回顾", "一文看懂", "排行榜",
                  ...
                  # 行业新闻 / 股票页 / 加盟招商类（不是活动）
                  "增长逻辑", "为何集体", "怎么选", "加盟", "研报", "财报", "招股",
                  "最新动态", "_f10_", "(Hk", "（Hk", "股价", "融资", "赛道", "内卷",
                  # 二手交易 / 电商 / 行业分析 / 负面新闻（都不是活动）
                  "未拆封", "包邮", "闲置", "二手", "转卖", "代购", "淘宝", "京东",
                  "闲鱼", "拼多多", "天猫", "直播带货",
...
def build_city_search_prompt(today, province, city_name, city_key, cutoff_days,
    ...
    return (
        ...
        "只要这4类：①新品上市（含上新/首发/回归/升级/限定）②IP联名/合作款 "
        "③送实物周边/物料（杯套、贴纸、冰箱贴、徽章、明信片、帆布袋等）④门店直享买一送一\n"
        "不要：纯折扣券/外卖套餐、没有赠品的纯打卡或门店照、行业新闻\n"
...
def llm_extract(city_name, today, materials):
    ...
    prompt = (
        ...
        "2. 只要这4类：①新品上市（含上新/首发/回归/升级/限定）②IP联名/合作款 "
        "③送实物周边或物料（杯套、贴纸、冰箱贴、徽章、明信片、帆布袋等）④门店直享买一送一；\n"
        "3. 不要：纯折扣券/外卖套餐、没有赠品的纯打卡或门店照、行业新闻、"
        "个人小店/食堂/社区团购、抽奖兑券、盘点/汇总/答疑类；\n"
        "4. 同一个活动只输出一条。\n"
        f"资讯：\n{materials}"
    )
